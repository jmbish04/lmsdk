...
describe("PromptRepository - findPrompts", () => {
  ...
  it("should return all active prompts by default", async () => {
    ...
    const prompts = await repository.findPrompts(createProjectId(1, 1));
  ...
  it("should exclude inactive prompts when activeOnly is true", async () => {
    ...
    await repository.deactivatePrompt(createEntityId(inactivePrompt.id, 1, 1));

    const prompts = await repository.findPrompts(createProjectId(1, 1), true);
  ...
  it("should include inactive prompts when activeOnly is false", async () => {
    ...
    const prompts = await repository.findPrompts(createProjectId(1, 1), false);
  ...
  it("should only return prompts for the specified tenant (cross-tenant protection)", async () => {
    ...
    const tenant1Prompts = await repository.findPrompts(createProjectId(1, 1));
    const tenant2Prompts = await repository.findPrompts(createProjectId(1, 2));
  ...
  it("should only return prompts for the specified project (cross-project protection)", async () => {
    ...
    const project1Prompts = await repository.findPrompts(createProjectId(1, 1));
    const project2Prompts = await repository.findPrompts(createProjectId(2, 1));
