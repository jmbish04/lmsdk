...
describe("GET /api/projects/:projectId/prompts/:promptId/versions", () => {
  ...
  it("returns list of prompt versions", async () => {
    ...
    expect(listPromptVersionsMock).toHaveBeenCalledWith(
  ...
  it("returns empty array when no versions exist", async () => {
    ...
    expect(listPromptVersionsMock).toHaveBeenCalledWith(
      expect.objectContaining({ id: 5, projectId: 1, tenantId: 1 })
    );
  });
  ...
  it("prevents cross-tenant access", async () => {
    ...
    expect(response.status).toBe(200);
    expect(listPromptVersionsMock).toHaveBeenCalledWith(
      expect.objectContaining({ id: 5, projectId: 2, tenantId: 1 })
    );
  });
});
