...
describe("DataSetRepository - findByTenantAndProject", () => {
  ...
  it("should return all non-deleted datasets for tenant and project", async () => {
    ...
    const datasets = await repository.findByTenantAndProject(projectId);
  ...
  it("should only return datasets for the specified tenant (cross-tenant protection)", async () => {
    ...
    const tenant1Datasets = await repository.findByTenantAndProject(projectId1);

		const projectId2 = new ProjectId(1, 2, '');
    const tenant2Datasets = await repository.findByTenantAndProject(projectId2);
  ...
  it("should only return datasets for the specified project (cross-project protection)", async () => {
    ...
    await repository.create({
      ...
      schema: "{}",
		...
		const projectId1 = new ProjectId(1, 1, '');
    const project1Datasets = await repository.findByTenantAndProject(projectId1);

		const projectId2 = new ProjectId(2, 1, '');
    const project2Datasets = await repository.findByTenantAndProject(projectId2);
