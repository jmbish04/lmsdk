...
describe("Datasets Routes - POST /api/projects/:projectId/datasets/:datasetId/logs", () => {
  ...
  it("should add logs to dataset", async () => {
    ...
    const [[entityId, logsInput]] = addLogsToDataSetMock.mock.calls;
    expect(entityId).toBeInstanceOf(EntityId);
    expect(entityId.id).toBe(10);
    expect(entityId.projectId).toBe(1);
    expect(entityId.tenantId).toBe(1);
    expect(logsInput).toEqual({ logIds: [101, 102] });
  });
  ...
  it("should parse log IDs as integers", async () => {
    ...
    expect(entityId.projectId).toBe(1);
    expect(entityId.tenantId).toBe(1);
    expect(logsInput).toEqual({ logIds: [101, 103] });
  });
