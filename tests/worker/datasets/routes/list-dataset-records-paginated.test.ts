...
describe("Datasets Routes - GET /api/projects/:projectId/datasets/:datasetId/records (paginated)", () => {
  ...
  it("should return paginated records", async () => {
    ...
    const [[entityId, pagination]] = listDataSetRecordsPaginatedMock.mock.calls;
    expect(entityId).toBeInstanceOf(EntityId);
    expect(entityId.id).toBe(1);
    expect(entityId.projectId).toBe(1);
  ...
  it("should enforce maximum page size", async () => {
    ...
    const response = await app.request(
      "/api/projects/1/datasets/1/records?pageSize=100",
      { method: "GET" },
      { DB: {} as any, PRIVATE_FILES: {} as any }
    );
    ...
    expect(entityId.projectId).toBe(1);
    expect(entityId.tenantId).toBe(1);
    expect(pagination).toEqual({ page: 1, pageSize: 100 });
  });
  ...
  it("should enforce minimum page size", async () => {
    ...
    const response = await app.request(
      "/api/projects/1/datasets/1/records?pageSize=1",
      { method: "GET" },
      { DB: {} as any, PRIVATE_FILES: {} as any }
    );
    ...
    expect(entityId.projectId).toBe(1);
    expect(entityId.tenantId).toBe(1);
    expect(pagination).toEqual({ page: 1, pageSize: 1 });
  });
  ...
  it("should enforce minimum page number", async () => {
    ...
    const response = await app.request(
      "/api/projects/1/datasets/1/records?page=1",
      { method: "GET" },
      { DB: {} as any, PRIVATE_FILES: {} as any }
    );
