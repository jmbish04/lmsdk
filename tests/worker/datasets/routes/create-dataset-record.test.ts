...
describe("Datasets Routes - POST /api/projects/:projectId/datasets/:datasetId/records", () => {
  ...
  it("should create a new record", async () => {
    ...
    const [[entityId, vars]] = createDataSetRecordMock.mock.calls;
  ...
  it("should use correct tenant ID (cross-tenant protection)", async () => {
    ...
    const [[createEntityId, vars]] = createDataSetRecordMock.mock.calls;
    expect(createEntityId).toBeInstanceOf(EntityId);
    expect(createEntityId.id).toBe(1);
    expect(createEntityId.projectId).toBe(1);
    expect(createEntityId.tenantId).toBe(2);
    expect(vars).toEqual(variables);
  });
