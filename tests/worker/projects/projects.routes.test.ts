...
describe("Projects Routes", () => {
  ...
  describe("GET /api/projects/:id", () => {
    it("returns a specific project by ID", async () => {
      ...
      expect(getProjectByIdMock).toHaveBeenCalledWith(expect.objectContaining({ id: 1, tenantId: 1 }));
    });
    ...
    it("returns 404 when project is not found", async () => {
      ...
      expect(getProjectByIdMock).toHaveBeenCalledWith(expect.objectContaining({ id: 999, tenantId: 1 }));
    });
    ...
    it("prevents cross-tenant access", async () => {
      ...
      const data = await response.json();
      expect(data).toEqual({ error: "Project not found" });
      expect(getProjectByIdMock).toHaveBeenCalledWith(expect.objectContaining({ id: 2, tenantId: 1 }));
    });
  });
  ...
  describe("DELETE /api/projects/:id", () => {
    it("deactivates an existing project", async () => {
      ...
      expect(getProjectByIdMock).toHaveBeenCalledWith(expect.objectContaining({ id: 1, tenantId: 1 }));
      expect(deactivateProjectMock).toHaveBeenCalledWith(expect.objectContaining({ id: 1, tenantId: 1 }));
    });
