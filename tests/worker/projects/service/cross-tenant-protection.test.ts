...
describe("ProjectService - Cross-tenant protection comprehensive tests", () => {
  ...
  it("should prevent any cross-tenant data leakage across all operations", async () => {
    ...
    for (const t1Project of tenant1Projects) {
      expect(await projectService.getProjectById(new ProjectId(t1Project.id, 1, "test-user"))).toBeDefined();
      expect(await projectService.getProjectById(new ProjectId(t1Project.id, 2, "test-user"))).toBeUndefined();
    }

    for (const t2Project of tenant2Projects) {
      expect(await projectService.getProjectById(new ProjectId(t2Project.id, 2, "test-user"))).toBeDefined();
      expect(await projectService.getProjectById(new ProjectId(t2Project.id, 1, "test-user"))).toBeUndefined();
    }
    ...
    await projectService.deactivateProject(new ProjectId(tenant2Projects[0].id, 1, "test-user")); // Wrong tenant
  ...
  it("should handle edge cases with tenantId values", async () => {
    ...
    for (const tenantId of edgeTenants) {
      ...
      // Verify isolation
      const retrieved = await projectService.getProjectById(new ProjectId(project.id, tenantId, "test-user"));
      ...
      for (const otherTenantId of edgeTenants) {
        if (otherTenantId !== tenantId) {
          const shouldBeUndefined = await projectService.getProjectById(
            new ProjectId(project.id, otherTenantId, "test-user")
          );
