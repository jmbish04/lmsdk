...
describe("ProjectService - deactivateProject", () => {
  ...
  it("should deactivate project when ID and tenantId match", async () => {
    ...
    await projectService.deactivateProject(new ProjectId(created.id, 1, "test-user"));
  ...
  it("should not deactivate project when tenantId does not match (cross-tenant protection)", async () => {
    ...
    await projectService.deactivateProject(new ProjectId(created.id, 2, "test-user"));
  ...
  it("should not affect other tenants' projects when deactivating", async () => {
    ...
    await projectService.deactivateProject(new ProjectId(tenant1Project.id, 1, "test-user"));
  ...
  it("should handle deactivating non-existent project gracefully", async () => {
    ...
    await expect(
      projectService.deactivateProject(new ProjectId(99999, 1, "test-user"))
  ...
  it("should enforce tenant isolation when deactivating by ID", async () => {
    ...
    for (let i = 0; i < projects.length; i++) {
      ...
      await projectService.deactivateProject(new ProjectId(project.id, wrongTenantId, "test-user"));
    ...
    for (const project of projects) {
      await projectService.deactivateProject(new ProjectId(project.id, project.tenantId, "test-user"));
